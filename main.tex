...
%In \S\ref{sec:discussion}, we explore modifications to the stellar models that affect the duration and intensity of the post-AGB phase and look at the UV properties of our models.
...
%\subsubsection{Post-AGB timescales}
%The variants tested here affect post-AGB timescales. Changes to either the mixing or mass loss efficiency will ultimately change the resultant envelope mass for stars in the post-AGB phase. For example, more efficient mass loss along the AGB implies that stars will depart the TP-AGB phase with less massive envelopes, higher luminosities, and more intense winds, which will decrease the post-AGB timescales.
%Post-AGB stars evolve at roughly constant luminosity to higher \Teff. The L-T evolution of post-AGB evolution depends strongly on stellar initial mass, where stars with higher initial masses evolve faster, and stars with lower initial mass spend more time at their peak luminosity. There is thus a characteristic range of initial stellar masses that dominate the post-AGB star flux output, typically stars with initial masses between 1.1 and 1.6\Msun.
% timescales of pAGB crossing of CMD vs. timescales of stars dominating ionizing photon budget -

%The last paragraph of S4.1 is very interesting.  First, can you quantify (even roughly) how much of a discrepancy there still is in the lifetimes?  You say “ultimately insufficient to resolve the tension entirely”, but it would be helpful to know if that means a factor of 2, 10%, or 10x.  What is also interesting in this context is Fig 7, where you show that DUP and mass-loss apparently have little effect on the lifetimes.   So then what could affect the lifetimes of the pAGB phase?  Aaron?  In other words, Fig 7 seems to suggest that the Ha emission is a fairly robust prediction of the models.  Maybe it's worth making this the conclusion to draw from Fig 7.

%One quick followup to #2 below.  I had a chat with Aaron about this, and it occurred to us that the number count / post-AGB lifetime issue might be somewhat orthogonal to the LINER emission lines due to post-AGBs.   At issue is which post-AGB stars (or rather which part of their evolution) is contributing most to the LINER emission.  The crossing time from cool-hot is so short that it is unlikely to play any role in the integrated emission.  I suspect that most of the ionizing budget is dominated by stars that are turning down toward the WD cooling track, since the lifetime spent there is much longer than the crossing time.  If this is correct, then I don't know if the CMD number counts have a direct connection to the LINER emission.  

%This may explain why you find so little dependence of the Ha EW on the stellar model ingredients.  Once the star has reached its hottest point and turns down to the WD cooling, I think the timescales are governed by very basic physics of energy loss from the proto-WD.  In contrast, the speed at which the pAGB star moves horizontally across the CMD depends on lots of things (core mass, envelope mass).  It may for example be the case that the different models Jieun ran for you do have quite different crossing timescales. 

%This may all be worth a short summarizing paragraph in the text.
...
%To summarize, our models produce emission line ratios and equivalent widths that agree well with those observed in LIER-like galaxies, with no fine tuning required. Moreover, consistent models are produced for a wide range of possible SFHs and ages, with only weak dependencies on metallicity and ionization parameter. Given how robust these predictions are, LIER-like emission should be nearly ubiquitous in quiescent galaxies with gas and a substantial post-AGB star population.
